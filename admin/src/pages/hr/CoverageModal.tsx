...
import type { Headquarter, Area, Beat, State } from '../../domain/hr/geography.types';
...
export function CoverageModal({
  ...
  const managerCascadeAreaIds = areas.filter((a) => allSupervisedHqIds.includes(a.hqId)).map((a) => a.id);
    ...
    <>
      ...
      <div className="actions" style={{ marginTop: '20px', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
        ...
        <button className="primary" onClick={handleSave} disabled={saving}>
          {saving ? 'Saving Coverage to D1...' : 'Save Territory Coverage'}
        </button>
      </div>
    </>
  );
