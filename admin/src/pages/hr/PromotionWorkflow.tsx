...
import { UserGateway } from '../../gateway/hr/userGateway';
...
export function PromotionWorkflow({
  ...
  useEffect(() => {
    if (activeTab === 'HISTORY') {
      ...
      UserGateway.getPromotionHistory().then((res) => setHistoryLogs(Array.isArray(res) ? res : [])).catch(() => setHistoryLogs([])).finally(() => setHistoryLoading(false));
    }
  }, [activeTab]);
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      setError(e?.message || 'Failed to execute role transition');
      setShowConfirmModal(false);
    } finally {
      setExecuting(false);
    }
  ...
  return (
    <>
      <Head
        ...
        sub="Elevate field staff (MR ➔ Sr. MR ➔ ASM), confirm probation periods (6M / 1Y) or reassign roles with live Cloudflare D1 updates."
      />
              ...
              {currentTarget && (
                <div style={{ background: '#f8fafc', padding: '14px', borderRadius: '10px', marginBottom: '16px', border: '1px solid #cbd5e1', fontSize: '13px' }}>
                  ...
                  <div style={{ color: '#475569', marginTop: '4px' }}>
                    Current Title: <b>{currentTarget.designation || currentTarget.role}</b> • Base HQ: <b>{getHqName(currentTarget.hqId)}</b> • Division: <b>{getDivisionName((currentTarget as any).divisionId || (currentTarget as any).division_id)}</b>
                  </div>
                </div>
              )}
