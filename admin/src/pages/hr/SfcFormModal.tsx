...
export function SfcFormModal({
  ...
  const allHqOptions = [
    ...
    ...hqs.map((h) => ({ v: h.id, l: `📍 ${h.name || h.hq_name} (${h.code || h.hq_code || 'HQ'})` })),
  ];
  ...
  const fetchGoogleMapsDistance = async () => {
    ...
    try {
      ...
      if (fromObj) { lat1 = fromObj.lat || fromObj.latitude; lon1 = fromObj.lng || fromObj.longitude; }
      if (toObj) { lat2 = toObj.lat || toObj.latitude; lon2 = toObj.lng || toObj.longitude; }

      const headers = { 'User-Agent': 'ChikuSFA-App/1.0 (contact@chikusfa.com)' };

      if (!lat1 || !lon1) {
        const fName = getHqName(fromHqId);
        const res1 = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(fName + ', Madhya Pradesh, India')}&format=json`, { headers });
        const d1 = await res1.json();
        if (d1 && d1.length > 0) { lat1 = d1[0].lat; lon1 = d1[0].lon; }
      }

      if (!lat2 || !lon2) {
        const tName = destMode === 'HQ' ? getHqName(targetToId) : getAreaName(targetToId);
        const res2 = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(tName + ', Madhya Pradesh, India')}&format=json`, { headers });
        const d2 = await res2.json();
        if (d2 && d2.length > 0) { lat2 = d2[0].lat; lon2 = d2[0].lon; }
      }
      ...
      if (lat1 && lon1 && lat2 && lon2) {
        ...
        if (routeData.routes && routeData.routes.length > 0) {
          roadKm = Math.round(routeData.routes[0].distance / 1000);
          setFetchSource('Google Maps / OSRM Highway Engine');
        }
      }
