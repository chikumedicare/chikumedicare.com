...
import { SfcGateway } from '../../gateway/hr/sfcGateway';
import { DaGateway } from '../../gateway/hr/daGateway';
...
export function SfcMaster({
  ...
  const handleSave = async (draft: Partial<SfcRate>) => {
    try {
      await SfcGateway.saveSfcRate(draft);
      await refreshList();
      return { success: true };
    } catch (err: any) {
      return { success: false, error: err?.error || err?.message || 'Failed to save SFC slab' };
    }
  };
  ...
  const handleDelete = async (id: string, routeName: string) => {
    if (window.confirm(`Are you sure you want to remove SFC fare route: ${routeName}?`)) {
      try {
        await SfcGateway.deleteSfcRate(id);
        await refreshList();
      } catch (err: any) {
        alert(err?.message || 'Failed to delete SFC slab');
      }
    }
  };
  ...
  return (
    <>
      ...
      <div className="panel table">
        {loading ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>Loading SFC Route Slabs & Live DA Rates from D1...</div>
        ) : (
          <table>
            <thead>
              <tr>
