...
export function TransferWorkflow({
  ...
  const currentDivisionId = (currentTarget as any)?.divisionId || (currentTarget as any)?.division_id || '';
  ...
  useEffect(() => {
    if (activeTab === 'HISTORY') {
      ...
      UserGateway.getTransferHistory()
  ...
  const filteredHqs = hqs.filter((h) => {
    ...
    const matchDiv = !destinationDivisionId || (h as any).divisionId === destinationDivisionId || (h as any).division_id === destinationDivisionId;
  ...
  const handleHqChange = (hqIdVal: string) => {
    ...
    if (selectedHq) {
      const hqDiv = (selectedHq as any).divisionId || (selectedHq as any).division_id;
      if (hqDiv) {
  ...
  const handleExecuteTransfer = async () => {
    ...
    try {
      await UserGateway.transferUser(
        currentTarget.id,
        newHqId,
      ...
      setError(e?.message || 'Failed to execute transfer on Cloudflare D1');
      setShowConfirmModal(false);
    } finally {
      setExecuting(false);
    }
            ...
            <Section title="Step 2: Transfer Reason & Parameters">
              ...
              <div style={{ marginTop: '14px', padding: '14px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '10px', fontSize: '13px' }}>
                ...
                <ul style={{ margin: '6px 0 0 16px', padding: 0, color: '#15803d', lineHeight: 1.5 }}>
                  ...
                  <li>Supervisor reporting chain updated in Cloudflare D1.</li>
                </ul>
              </div>
