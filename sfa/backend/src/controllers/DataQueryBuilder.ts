...
// Table-specific allowed columns for sorting and filtering
const ALLOWED_SORT_COLUMNS: Record<string, Set<string>> = {
...
export class DataQueryBuilder {
  static buildSelectQuery(
    ...
    // 7. Search parameter binding
    if (search && search.trim().length > 0) {
      const sanitizedSearch = search.trim();
      if (collection === 'users') {
        query += ` AND (full_name LIKE ? OR emp_code LIKE ? OR mobile LIKE ?)`;
        queryParams.push(`%${sanitizedSearch}%`, `%${sanitizedSearch}%`, `%${sanitizedSearch}%`);
        ...
        queryParams.push(`%${sanitizedSearch}%`, `%${sanitizedSearch}%`, `%${sanitizedSearch}%`, `%${sanitizedSearch}%`);
      } else {
        query += ` AND name LIKE ?`;
